...
func instantiate_player_scene(unique_id, player_info):
    ...
    if multiplayer.is_server():
        ...
        var data = {"id": unique_id, "info": player_info, "position": PLAYER_INIT_POSITION + len(players) * Vector2(50, 0)} # Adding a small offset to avoid overlap
        player_spawner.spawn(data) # Spawning players with a small offset to avoid overlap
